...
def isbns_to_gapi(isbn_list,keys_to_extract):
    output_list = []
    for i in isbn_list:
        my_query = ('https://www.googleapis.com/books/v1/volumes?q=isbn:'
                    + i
                    + '&key='
                    + GOOGLE_KEY)
        response = requests.get(my_query)
        book = json.loads(response.text)
        output_list.append(book['items'][0])
    process_list(output_list,keys_to_extract)
    return output_list
